...
public class RenderExtension {
	...
	protected boolean drawQuadChecker()
	{
		return true;
	}
	...
	protected void rotateQuad(int x, int y, int x2, int y2, double angle)
	{
		...
		drawFreeQuad(x, y, (int)topRightX, (int)topRightY, (int)bottomLeftX, (int)bottomLeftY, (int)bottomRightX, (int)bottomRightY);
		
	}
	//rotate by the topleft corner as axis of rotation
	protected void drawRectangle(int originX, int originY, int width, int height, double angle)
	{
		...
		drawFreeQuad((int)originX, 
					 (int)originY, 
					 (int)newTopRightX+originX, 
					 (int)newTopRightY+originY, 
					 (int)newBottomLeftX+originX, 
					 (int)newBottomLeftY+originY, 
					 (int)newBottomRightX+originX, 
					 (int)newBottomRightY+originY);
	...
	{
		double radians = Math.toRadians(angle);
		drawRectChecker(x, y, (int)(width*Math.cos(radians)), (int)(height*Math.sin(radians)));
	}
	...
	protected void drawHUD()
	{
		...
		drawRect(gameWidth-200, 20, 180, 40);
	}
}
